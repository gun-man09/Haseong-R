...
function usePlayerItem(it){
  ...
  switch(it){
    ...
    case "케이크": player.hp=Math.min(player.maxHp,player.hp+1); break;
...
function aiUseItem(item){
  ...
  switch(item.name){
    ...
    case "케이크": ai.hp=Math.min(ai.maxHp,ai.hp+1); message.textContent="🎂 AI 케이크 사용! 체력 +1"; updateUI(); break;
...
function aiTurn(){
  ...
  if(round>=2 && Math.random()<0.5){
    const itemPool=[
      {name:"톱날", used:false},{name:"돋보기", used:false},{name:"케이크", used:false},
...
function setupRound(){
  ...
  playerItems=round>=2?["톱날","돋보기","케이크","맥주","수갑"]:[]; // 2라운드부터 아이템 지급
